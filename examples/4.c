...
void mat4_rotate_z(float* m, float angleDeg){
    ...
    float m0=m[0], m4=m[4], m1=m[1], m5=m[5];
...
/*---------------- Node Tree ----------------*/
Node* create_node(float x,float y,int renderable,int rotate,float speed,float sx,float sy){
    Node* n = (Node*)malloc(sizeof(Node));
    n->x=x; n->y=y; n->rotation=0;
    n->scaleX=sx; n->scaleY=sy;
    n->renderable=renderable; n->rotate=rotate; n->rotation_speed=speed;
    n->child_count=0;
    for(int i=0;i<MAX_NODES;i++) n->children[i]=NULL;
    return n;
}

void add_child(Node* parent, Node* child){
    if(parent->child_count<MAX_NODES) parent->children[parent->child_count++] = child;
}
...
void render_node(Engine* engine, Node* node, float* parentMat){
    if(!node) return;
    float model[16];
    for(int i=0;i<16;i++) model[i]=parentMat[i];
...
void render_scene(Engine* engine, Scene* scene){
    ...
    mat4_rotate_z(model,scene->rotation);
...
Game* create_game(){
    ...
    g->state=0; g->scenes=NULL; g->scene_count=0; g->current_scene_index=0;
    return g;
}

void add_scene(Game* g, Scene* s){
    g->scenes = (Scene**)realloc(g->scenes,sizeof(Scene*)*(g->scene_count+1));
    g->scenes[g->scene_count++] = s;
}

Scene* get_current_scene(Game* g){
    if(g->scene_count==0) return NULL;
    return g->scenes[g->current_scene_index];
}
...
void engine_eventHandler(Engine* engine){
    ...
    // trigger rotation for individual nodes
...
void engine_started(Engine* engine){
    setupShaders(engine);
    Node* root = create_node(0.0f,0.0f,0,0,0,1.0f,1.0f);
    Node* c1 = create_node(0.3f,0.0f,1,0,2.0f,0.5f,0.5f);
    Node* c2 = create_node(-0.3f,0.0f,1,0,1.5f,0.5f,0.5f);
    Node* gc = create_node(0.0f,0.2f,1,0,3.0f,0.3f,0.3f);
    add_child(c1,gc);
    add_child(root,c1);
    add_child(root,c2);
