...
typedef void(*CollisionCallback)(Node*, Node*);
...
struct Node {
    ...
    float color[3];
    float size;  // size for collision
    Node* children[MAX_NODES];
    int child_count;
    CollisionCallback onCollision; // custom event
};
...
/*---------------- Matrix Helper ----------------*/
void mat4_identity(float* m){ for(int i=0;i<16;i++) m[i]=(i%5==0)?1.0f:0.0f; }
void mat4_translate(float* m,float x,float y,float z){ m[12]+=x; m[13]+=y; m[14]+=z; }
void mat4_scale(float* m,float sx,float sy,float sz){ m[0]*=sx; m[5]*=sy; m[10]*=sz; }
void mat4_rotate_z(float* m,float angleDeg){
    float rad=angleDeg*0.0174533f;
    float c=cosf(rad), s=sinf(rad);
    float m0=m[0], m1=m[1], m4=m[4], m5=m[5];
    m[0]=c*m0+s*m1; m[1]=-s*m0+c*m1; m[4]=c*m4+s*m5; m[5]=-s*m4+c*m5;
}

/*---------------- Shader ----------------*/
const char* vertexShaderSrc="attribute vec2 aPos;\nuniform mat4 uModel;\nvoid main(){gl_Position=uModel*vec4(aPos,0.0,1.0);}";
...
/*---------------- Node Tree ----------------*/
Node* create_node(float x,float y,int renderable,int rotate,float speed,float sx,float sy,float r,float g,float b,float size,CollisionCallback cb){
    Node* n=(Node*)malloc(sizeof(Node));
    n->x=x; n->y=y; n->rotation=0;
    n->scaleX=sx; n->scaleY=sy; n->renderable=renderable; n->rotate=rotate; n->rotation_speed=speed;
    n->moveSpeedX=0.002f; n->moveSpeedY=0.002f; n->moveDirX=1; n->moveDirY=1;
    n->color[0]=r; n->color[1]=g; n->color[2]=b;
    n->size=size; n->onCollision=cb;
...
void check_collision(Node* a, Node* b){
    float dx=a->x-b->x;
    float dy=a->y-b->y;
    float dist=sqrtf(dx*dx+dy*dy);
    if(dist<(a->size+b->size) && a->onCollision) a->onCollision(a,b);
}
...
void animate_node(Node* node){
    ...
    // Collision check with siblings
    for(int i=0;i<node->child_count;i++)
        for(int j=i+1;j<node->child_count;j++)
            check_collision(node->children[i],node->children[j]);
}
...
/*---------------- Collision Callback ----------------*/
void onNodeCollision(Node* a, Node* b){
    printf("Collision detected between nodes! Node colors: [%.1f %.1f %.1f] & [%.1f %.1f %.1f]\n",
...
void engine_started(Engine* engine){
    ...
    Node* root=create_node(0.0f,0.0f,0,0,0,1.0f,1.0f,1.0f,1.0f,1.0f,0.1f,NULL);
    Node* c1=create_node(0.2f,0.0f,1,0,2.0f,0.5f,0.5f,1,0,0,0.1f,onNodeCollision);
    Node* c2=create_node(-0.2f,0.0f,1,0,1.5f,0.5f,0.5f,0,1,0,0.1f,onNodeCollision);
    add_child(root,c1); add_child(root,c2);

    Scene* scene=(Scene*)malloc(sizeof(Scene)); scene->root=root; scene->rotation=0;
...
void engine_destroy(Engine* engine){ if(engine->context) SDL_GL_DestroyContext(engine->context); if(engine->window) SDL_DestroyWindow(engine->window); SDL_Quit(); }
void engine_getevent(Engine* engine){ SDL_Event e; while(SDL_PollEvent(&e)){ engine->event=e; if(e.type==SDL_EVENT_QUIT) engine->run=0; } }
